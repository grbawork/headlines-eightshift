...
export const WrapperOptions = ({ attributes, setAttributes }) => {
	const {
		label = __('Spacing & layout', 'headlines'),
		showWrapperId = true,
	...
	let wrapperUseOptions = [
		{
			label: __('Just the block', 'headlines'),
			value: 'off',
	...
	if (wrapperSimpleShowControl) {
		wrapperUseOptions = [...wrapperUseOptions, {
			label: __('Spacing only', 'headlines'),
			value: 'simple',
					...
					{!wrapperNoControls && wrapperUseShowControl &&
						<div className='es-display-flex es-items-center es-gap-1 es-ml-auto es-flex-shrink-0 es-mr-1 es-wrapper-options-copy-paste'>
							<Button
								...
								label={__('Copy configuration', 'headlines')}
								showTooltip
							/>
							...
							<Button
								...
								className='es-button-icon-24 es-button-square-24 es-rounded-0.75!'
								label={__('Paste configuration', 'headlines')}
								showTooltip
							/>
						</div>
					}
			...
			{wrapperUse &&
				<>
					...
					{wrapperUse && showWrapperTag &&
						<OptionSelector
							label={__('Wrapper tag', 'headlines')}
							icon={icons.code}
					...
					>
						...
						{(!noLeftSpacingInSelector || !noRightSpacingSelector) &&
							<Collapsable label={__('More', 'headlines')} icon={icons.moreH}>
								{!noLeftSpacingSelector &&
									<ResponsiveNumberPicker
										icon={icons.spacingLeft}
										label={__('Left', 'headlines')}
						...
						{showWrapperSpacingBottomIn &&
							<ResponsiveNumberPicker
								...
								label={__('Bottom inner', 'headlines')}
						...
						{(!noLeftSpacingInSelector || !noRightSpacingInSelector) &&
							<Collapsable label={__('More', 'headlines')} icon={icons.moreH} noBottomSpacing>
								{!noLeftSpacingInSelector &&
									<ResponsiveNumberPicker
										icon={icons.spacingLeftIn}
										label={__('Left inner', 'headlines')}
								...
								{!noRightSpacingInSelector &&
									<ResponsiveNumberPicker
										...
										label={__('Right inner', 'headlines')}
					...
					<Section
						...
						icon={icons.design}
						label={__('Design', 'headlines')}
					>
						{showWrapperBgColorPicker &&
							<AdvancedColorPicker
								icon={icons.backgroundType}
								label={__('Background', 'headlines')}
								...
								types={
									[
										{
											label: __('None', 'headlines'),
											value: '',
											icon: icons.emptyCircle,
										},
										{
											label: __('Project color', 'headlines'),
											value: 'project',
											icon: icons.colorAlt,
										}
									]
						...
						{(showWrapperDividerTop || showWrapperDividerBottom || showWrapperDividerLeft || showWrapperDividerRight || showWrapperDividerColor) &&
							...
							>
								{showWrapperDividerColor &&
									<ColorPicker
										colors={[{ name: __('None', 'headlines'), slug: undefined, color: 'transparent' }, ...dividerColors]}
										...
										pickerPopupTitle={__('Divider color', 'headlines')}
										label={__('Color', 'headlines')}
										icon={icons.colorAlt}
									/>
								}
											...
											{breakpointNames.map((breakpoint, index) => {
												...
												return (
													<Button
														...
														label={attributeValue ? __('On', 'headlines') : __('Off', 'headlines')}
														showTooltip
													/>
												);
											})}
					...
					<Section
						...
						icon={icons.tools}
						label={__('Advanced', 'headlines')}
						subtitle={__('Visibility, anchor, ID', 'headlines')}
						noBottomSpacing
						collapsable
					>
						{showWrapperHide &&
						...
						{showWrapperAnchorId &&
							<TextControl
								label={<IconLabel icon={icons.anchor} label={__('Block Anchor ID', 'headlines')} />}
						...
						{showWrapperId &&
							<TextControl
								label={<IconLabel icon={icons.id} label={__('Block unique identifier', 'headlines')} />}
								value={wrapperId}
