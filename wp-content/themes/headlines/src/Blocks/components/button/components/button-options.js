...
export const ButtonOptions = (attributes) => {
	...
	return (
		<UseToggle {...generateUseToggleConfig(attributes, manifest, 'buttonUse')}>
			...
			<Section showIf={!hideAriaLabel} icon={icons.a11y} label={__('Accessibility', 'headlines')} collapsable reducedBottomSpacing>
				<TextControl
					label={<IconLabel icon={icons.ariaLabel} label={__('ARIA label', 'headlines')} />}
					value={buttonAriaLabel}
					onChange={(value) => setAttributes({ [getAttrKey('buttonAriaLabel', attributes, manifest)]: value })}
					help={__('Description of the button.', 'headlines')}
				/>
			</Section>

			<Section showIf={!hideId} icon={icons.tools} label={__('Advanced', 'headlines')} collapsable noBottomSpacing>
				<TextControl
					label={<IconLabel icon={icons.id} label={__('Unique identifier', 'headlines')} />}
					value={buttonId}
