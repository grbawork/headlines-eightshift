...
export const SocialNetworksOptions = (attributes) => {
	...
	const modeOptions = [
		{
			label: __('Link to social networks', 'headlines'),
			icon: icons.link,
			value: false,
		},
		{
			label: __('Share to social networks', 'headlines'),
			icon: icons.share,
			value: true,
		}
	];

	const pickerSectionTitle = socialNetworksShareMode ? __('Share targets', 'headlines') : __('Social networks', 'headlines');
