...
export const SiteNavigationEditor = ({ attributes, setAttributes }) => {
	...
	const baseLinkClass = 'es-border-cool-gray-100 es-cursor-pointer es-bg-none es-pl-2 es-pr-1.25 es-py-1 es-rounded-2';
	...
	const siteNavigationLinks = checkAttr(
		'siteNavigationLinks',
		attributes,
		manifest
	);

	const siteNavigationButtonsClass = classnames([
		selector(blockClass, blockClass, 'buttons-wrapper'),
	]);
	...
	return (
		<div className={blockClass}>
			...
			<div className={linksClass}>
							...
							<ReOrderableItem
								...
								preIcon={
									<div className="es-display-flex">
										<button
											onClick={() => {
												const newValue = [...siteNavigationLinks].filter(
													(_, index) => index !== i
												);
												setAttributes({
													[getAttrKey(
				...
				<div className={siteNavigationButtonsClass}>
					<ButtonEditor
						{...props('buttonPrimary', attributes, {
							setAttributes,
							blockClass: blockClass,
							selectorClass: 'btn-primary',
						})}
					/>
