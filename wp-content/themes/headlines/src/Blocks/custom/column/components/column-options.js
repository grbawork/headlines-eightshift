...
export const ColumnOptions = ({ attributes, setAttributes }) => {
	...
	return (
		<PanelBody title={__('Column', 'headlines')}>
			<WidthOffsetRangeSlider
				{...generateWidthOffsetRangeSliderConfig({
			...
			<Section icon={icons.alignHorizontalVerticalAlt} label={__('Alignment', 'headlines')}>
				<Responsive
					label={__('Horizontal', 'headlines')}
			...
			<Section icon={icons.tools} label={__('Advanced', 'headlines')} noBottomSpacing>
				<ResponsiveToggleButton
					{...generateResponsiveToggleButtonConfig({
				...
				<Responsive
					label={<IconLabel icon={icons.order} label={__('Order', 'headlines')} />}
				...
				>
					{getDefaultBreakpointNames().map((breakpoint, index) => {
						...
						let options = [
							{ label: __('Auto', 'headlines'), value: true, icon: icons.automatic },
							{ label: __('Manual', 'headlines'), value: false, icon: icons.pointerHand },
						];
						...
						return (
							<Fragment key={index}>
								<Control
									help={
										isAuto
											? __('Follows the order of elements in the editor', 'headlines')
											: __('Forces column order, independent of the element order in the editor', 'headlines')
									}
									noBottomSpacing
								>
								...
								<AnimatedContentVisibility showIf={!isAuto && index === 0} additionalContainerClasses='es-mt-2'>
									<Notification
										type='info'
										text={__('Note', 'headlines')}
										subtitle={__('Make sure to set the order all other columns in the same Columns block', 'headlines')}
										noBottomSpacing
									/>
								</AnimatedContentVisibility>
							</Fragment>
