 ...
 *
 * @package Headlines\AdminMenus
 */

declare(strict_types=1);

namespace Headlines\AdminMenus;

use HeadlinesVendor\EightshiftLibs\AdminMenus\AbstractAdminMenu;
use HeadlinesVendor\EightshiftLibs\Helpers\Components;
use WP_Query;

/**
 * ReusableBlocksHeaderFooter class.
 */
class ReusableBlocksHeaderFooter extends AbstractAdminMenu
{
	/**
	...
	{
		return \esc_html__('Header & footer', 'headlines');
	}
	...
	public function renderPartialSelector($args): void
	{
		...
		if ($reusableBlocksQuery->have_posts()) {
			...
			<select id="<?php echo \esc_attr($args['label_for']); ?>" name="<?php echo \esc_attr($type); ?>">
				<option value="">
					&mdash; <?php \esc_html_e('None', 'headlines'); ?> &mdash;
				</option>
		...
		<?php } else { ?>
			<i><?php echo \esc_html__('No reusable blocks found.', 'headlines'); ?></i>
			<?php
		}
	}
